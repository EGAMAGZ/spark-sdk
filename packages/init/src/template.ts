...
const counterTs = `export function setupCounter(element: HTMLButtonElement) {
  let counter = 0;
...
export function getTemplateFiles(projectName: string): TemplateFile[] {
  return [
    ...
    { path: 'src/counter.ts', content: counterTs },
    { path: 'src/style.css', content: styleCss },
    { path: 'src/typescript.svg', content: typescriptSvg },
    { path: 'src/vite-env.d.ts', content: viteEnvDts },
    { path: 'public/vite.svg', content: viteSvg },
    { path: 'pages/counter.html', content: counterHtml },
  ];
}
